...
### Bug close-date override
- Table `bug_close_override` (`key` PK, `closed_date`) — `drizzle/0014_bug_close_override.sql`, declared in `src/server/db/schema.ts`. Manual rows only (no API/UI); insert by SQL. Read through a 5-minute `MemoryCache`; `invalidateCloseOverrides()` exported for a future write path.
- `bug-monitoring.repository.ts` → `applyCloseOverrides()` rewrites `fields.resolutiondate` inside the private `fetchBugs()`, so BOTH `fetchBugsByBoard` and `fetchActiveBugsByBoardAtMonthEnd` — and every consumer (bug monitoring UI, productivity summary `loadBugDoneCount`/`loadBugRaisedCount`, `countActiveBugsAtMonthEnd`) — see the corrected date. Key present = override wins; absent = Jira's `resolutiondate` (null = still open, uncounted).
- Operator material: `apps/tere-project/docs/database/bug-close-override-seed.sql` (paste-and-run DDL + current 67 overrides, idempotent) regenerated by `apps/tere-project/scripts/bug-close-override-seed.mjs` from the "Incident Close" CSV export (`Tiket,Close Date` in M/D/YYYY).
- Caveat: `buildBugSnapshotJql` filters `resolutiondate` server-side in JQL, so an override cannot pull a bug back into a snapshot page Jira already excluded. Only affects the JQL-prefiltered month-end path, not the full-history counts.
