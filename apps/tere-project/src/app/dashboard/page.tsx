...
export default function Dashboard() {
  ...
  return isLead
    ...
    : <MemberDashboard teams={teams} summaryLoading={summaryLoading} message={message} />;
}
...
/* ── Sprint Card Stack ── */
function SprintCardStack({ teams }: { teams: { teamName: string; sprintName: string | null }[] }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'relative', cursor: 'pointer', height: 28 }}>
        ...
        <span style={{
          ...
          fontSize: 10, color: T.accent, fontWeight: 700, fontFamily: mono,
        }}>
          +{sprints.length - 1}
        </span>
      </div>
        ...
        <div style={{
          ...
          {sprints.map((s, i) => (
            ...
            >
              ...
              <span style={{ fontSize: 12, color: T.titleCol, fontFamily: sans, fontWeight: 500 }}>
                {s.sprint}
              </span>
            </div>
          ))}
...
function LeadDashboard({ teams, bugBoards, members, boardShortNameMap, summaryLoading, message }: {
  ...
  // Sprint date range from the teams data
  const sprintDates = teams
    .filter(t => t.sprintStartDate && t.sprintEndDate)
    .map(t => `${t.sprintStartDate} → ${t.sprintEndDate}`);
    ...
    <div style={{ paddingBottom: 24 }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ marginTop: 6, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          <SprintCardStack teams={teams} />
          {sprintDates.length > 0 && (
            <span style={{ color: T.subCol, fontSize: 12, fontFamily: mono }}>
              {sprintDates[0]}
              {teams[0]?.totalWorkingDays ? ` · ${teams[0].totalWorkingDays} working days` : ''}
            </span>
          )}
        </div>
      </div>
...
/* ── MEMBER DASHBOARD ── */
function MemberDashboard({ teams, summaryLoading, message }: {
  teams: (TeamSummary & { isLoading: boolean; error: Error | null })[];
  summaryLoading: boolean;
  message: string;
}) {
