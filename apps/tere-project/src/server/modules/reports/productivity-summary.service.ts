...
export function buildProductivitySummaryRangeSheet(data: ProductivitySummaryRangeResponse) {
  ...
  const values = [
    ...
    [],
    ['Month', 'Source', 'Metric Basis', 'Active Members', 'Productivity', 'Total Bugs', 'Bugs Raised'],
    ...data.chart.map(point => [point.month, point.source, point.metricBasis, available(point.activeMembers), available(point.productivityMetric), available(point.bugsTotal), available(point.bugsRaised)]),
    [],
    ['Details'],
