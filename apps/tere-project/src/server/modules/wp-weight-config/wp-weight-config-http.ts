...
export type WpWeightConfigApiErrorCode =
  | 'UNAUTHORIZED'
  | 'FORBIDDEN'
...
function fixedError(
  code: WpWeightConfigApiErrorCode,
  message: string,
  status: number,
): Response {
...
export function errorResponse(error: unknown): Response {
  if (error instanceof WpWeightConfigError) {
...
export function normalizeWpResponse(response: Response): Response {
